...
export async function computeSprintNetwork(input: {
  ...
  if (data.history.length < MIN_CLOSED_SAMPLES) {
    return {
      ok: false,
      reason: 'insufficient_data',
  ...
  const nodes: NetworkNode[] = data.remaining.map((r) => {
    ...
    sources.set(r.taskId, resolved.source)
    return { id: r.taskId, estimate: resolved.estimate, dependsOn: dependsOn.get(r.taskId) ?? [] }
  })
  ...
  const taskViews: NetworkTaskView[] = data.remaining.map((r) => {
    const a = analyzedById.get(r.taskId)!
    return {
      taskId: r.taskId,
      title: r.title,
      storyPoints: r.storyPoints,
      estimate: nodeById.get(r.taskId)!.estimate,
      estimateSource: sources.get(r.taskId)!,
      ...
      critical: a.critical,
      dependsOn: dependsOn.get(r.taskId) ?? [],
    }
  })
...
function computeHorizonDays(sprint: {
  state: SprintState
