...
2. Для каждой из 1000 итераций: `H` раз сэмплируется день `d_i ~ D` (with replacement), `total += d_i`; цикл рвётся, как только `total ≥ N` (закрыто достаточно задач). Записываем `completionDays` = индекс дня раннего выхода (1-indexed); если за весь горизонт `H` цикл не оборвался — `completionDays = H + 1` (signal-значение «не успели за горизонт»).
3. Вероятность = доля итераций, где `total ≥ N` за ≤ H дней.
4. Перцентили `completionDays` (P50 / P85 / P95) строятся по этому ряду из 1000 значений — сколько дней ушло на накопление N, с верхней границей `H + 1` для итераций, которые не уложились.

**Почему bootstrap, а не среднее × дни.** Простейшая детерминированная альтернатива — `mean(throughput) × horizonDays` — даёт точечную оценку без распределения. При variance ≥ mean (типично для команд 5–10 человек: фоновое отвлечение, отпуска, инциденты) такая оценка системно занижает риски: клиенту обещаешь дату, на которой реальная вероятность ≈ 50%, а не 85% / 95%, к которым apellируют менеджеры. Bootstrap наоборот возвращает полное эмпирическое распределение — тот же исторический ряд, но без предположений о форме (нормальность, экспоненциальность); процентили P50 / P85 / P95 явно показывают spread, а не маскируют его средним.
...
- **Пороги данных:** `MIN_DAYS_OF_HISTORY = 14` (вектор throughput должен покрыть ≥ 14 дней) и `totalClosed > 0` (за 90 дней должна быть закрыта хотя бы одна задача). Иначе возвращается `{ ok: false, reason: 'insufficient_data', sampleDays, requiredDays: 14 }` — UI не получает «вероятность 0%», полученную из шума.
