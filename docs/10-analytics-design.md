...
**Почему bootstrap, а не среднее × дни.** Простейшая детерминированная альтернатива — `mean(throughput) × horizonDays` — даёт точечную оценку без распределения. При variance ≥ mean (типично для команд 5–10 человек: фоновое отвлечение, отпуска, инциденты) такая оценка системно занижает риски: клиенту обещаешь дату, на которой реальная вероятность ≈ 50%, а не 85% / 95%, к которым апеллируют менеджеры. Bootstrap наоборот возвращает полное эмпирическое распределение — тот же исторический ряд, но без предположений о форме (нормальность, экспоненциальность); процентили P50 / P85 / P95 явно показывают spread, а не маскируют его средним.
